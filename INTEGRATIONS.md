...
- **Status:** ✅ Values collected 2026-06-15. ✅ **Rotated 2026-06-23** — leaked client secret + refresh token revoked: deleted the "Craftons Ads" OAuth grant, disabled the leaked secret in Cloud Console (added a new secret), re-minted the refresh token via OAuth Playground, placed the two new values in **Vercel** env vars + redeployed. ✅ **Mirrored into this engine's env + VERIFIED LIVE 2026-06-30** — all six vars present, OAuth refresh works, **Basic access confirmed** by reading the non-test advertiser account. `tools/google-ads.mjs` (read-only) built and runs here. ⏳ Remaining (desktop tidy): delete the disabled old secret in Cloud Console now that the rotated creds are verified working.
