...
# Usage:  ./render.sh            → out/craftons-howto-setout.mp4
...
case "${1:-render}" in
  check)    "$HF" check ;;
  snapshot) "$HF" snapshot --at 1.2,3.6,9.6,12.6,18.2,21.8,25.2,28.6 --no-end --describe false -o snapshots ;;
  *)        mkdir -p out && "$HF" render -o "out/craftons-howto-setout.mp4" ;;
esac
