...
4. `python3 video/captions-highlight.py reel.words.json caps.ass` — the approved Craftons style (green Inter Bold, 2 lines x 2 words, spoken word in line green with `BOX=0 HL=linegreen`, or white in a green box by default; `RAISE=t:px` lifts it over on-page buttons).
